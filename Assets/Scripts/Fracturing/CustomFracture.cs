...
public class CustomFracture : MonoBehaviour
{
    ...
    [SerializeField] private bool inheritLayer;
    
    [SerializeField] private bool addSlowDestroy;
        ...
        if (triggerOptions.triggerType == TriggerType.Collision)
        {
            if (collision.contactCount > 0)
            {
                ...
                if (collisionForce > triggerOptions.minimumCollisionForce &&
                   ((triggerOptions.filterCollisionsByTag && tagAllowed) || !triggerOptions.filterCollisionsByTag))
                {
    ...
    void OnTriggerEnter(Collider collider)
    {
        if (triggerOptions.triggerType == TriggerType.Trigger)
        {
            ...
            if ((triggerOptions.filterCollisionsByTag && tagAllowed) || !triggerOptions.filterCollisionsByTag)
            {
    ...
    private GameObject CreateFragmentTemplate()
    {
        ...
        // Copy the layer of the object
        if (inheritLayer)
        {
            obj.layer = this.gameObject.layer;
        }

        if (addSlowDestroy && destroyTime > 0)
        {
            SlowDestroy destruction = obj.AddComponent<SlowDestroy>();
            destruction.destroyTime = destroyTime;
        }
